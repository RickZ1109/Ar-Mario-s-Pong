...
void drawCapa() {
  ...
  float textWidth = textWidth("PLAY GAME");
  float textHeight = textAscent() + textDescent();
  float textX = width / 1.5 - 25 - textWidth / 2;
  float textY = height / 1.5 - 5 - textHeight / 2;
  text("PLAY GAME", width / 1.5 - 25, height / 1.5 - 5);
  
  // Verifica clique no botão "PLAY GAME"
  if (mousePressed && mouseX > textX && mouseX < textX + textWidth && mouseY > textY && mouseY < textY + textHeight) {
    gameState = "menu";
  }
}
...
void mousePressed() {
  ...
  } else if (gameState.equals("capa")) {
    // Verifica se o clique está dentro da área do texto "PLAY GAME"
    textSize(17);
    float textWidth = textWidth("PLAY GAME");
    float textHeight = textAscent() + textDescent();
    float textX = width / 2 - textWidth / 2;
    float textY = height / 2 - textHeight / 2;
    if (mouseX > textX && mouseX < textX + textWidth && mouseY > textY && mouseY < textY + textHeight) {
      gameState = "menu";
    }
  }  else if (gameState.equals("playing")) {
    // Verifica se o clique está dentro da área do botão de pausa
    float pauseButtonX = width - 30; // Posição X do botão de pausa
    float pauseButtonY = height - 30; // Posição Y do botão de pausa
    float pauseButtonWidth = 15; // Largura do botão de pausa
    float pauseButtonHeight = 20; // Altura do botão de pausa
